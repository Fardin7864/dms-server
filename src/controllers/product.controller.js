...
const getProduct = asyncHandler(async (req,res) => { 
 ...
 const addProducts = asyncHandler (async (req,res) => { 
  ...
  const updateProduct = asyncHandler(async (req,res) => { 
        ...
        if([productName, category, company, wet].some(field => field.trim() === "")) throw new ApiError(400, "ProductName, price, category, company and wet required!")
   ...
   const deleteProduct = asyncHandler(async (req,res) => { 
    ...
    return res.status(200).json(new ApiResponse(200, deleteProduct, "Deleted successfully!"))
    } catch (error) {
